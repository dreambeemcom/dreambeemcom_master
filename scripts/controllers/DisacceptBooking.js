...
angular.module('dreambeemcomApp')
    .controller('DisAcceptBookingCtrl', function ($scope, $window, $timeout, stkHttpService, Facebook, $mdToast, $location, $route) {
                ...
                if(data.data[0].bookedIdKind === 'Facebook') {
                    Facebook.getLoginStatus(function(response) {
                        ...
                        if (response.status === 'connected') {
                            var stkService = stkHttpService.facebookDisAcceptBookingConfirmation(bid);
                            stkService.then(function(data1) {
                                ...
                                if($scope.response.message === 'ok') {
                                    $mdToast.show({
                                        ...
                                        templateUrl : 'views/toasts/toast_facebookBooking_disconfirmation_1.html',
                    ...
                    stkService.then(function(data2) {
                        ...
                        if($scope.response.data === 'ok') {
                            var stkService = stkHttpService.twitterDisAcceptBookingConfirmation(bid);
                            stkService.then(function(data3) {
                                ...
                                if($scope.response.message === 'ok') {
                                    $mdToast.show({
                                        ...
                                        position    : 'top',
                                        controller  : 'ToastCtrl',
                                        templateUrl : 'views/toasts/toast_twitterBooking_disconfirmation_1.html',
                                        toastClass  : 'md-success-toast-theme'
                                    });

                                    $scope.booking_confirmation = 'Twitter Buchungsablehnung läuft';
                                    $scope.error_confirmation = false;
                                    ...
                                    $scope.booking_confirmation = 'Twitter Buchungsablehnung fehlgeschlagen. Wiederhole noch einmal die Ablehnung.';
                                    $scope.error_confirmation = true;
                                }
